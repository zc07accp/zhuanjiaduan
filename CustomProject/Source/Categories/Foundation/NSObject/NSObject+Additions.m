    ...
    for (NSString* key in keys)
    {
        if ([object isKindOfClass:[NSArray class]])
        {
            ...
            if (range.location != NSNotFound)
            {
                ...
                if (!validIndex)
                {
                    NSString* error = [NSString stringWithFormat:@"Index:%li out of bounds for valueAtKeyPath:@\"%@\" for array:0x%@.count = %lu", (long)index, keyPath, object, (unsigned long)[object count]];
