...
DEF_SINGLETON(BaseHttp)
/**  超时时间 */
static NSTimeInterval   requestTimeout = 30.f;
/**  网络状态 */
static NetworksStatus   _netStatus;
/**  请求任务 */
    ...
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        switch (status)
        {
            case AFNetworkReachabilityStatusUnknown:
                ...
                [BaseHttp sharedInstance].networkState=@"no";
                break;
            ...
            case AFNetworkReachabilityStatusReachableViaWWAN:
                ...
                [BaseHttp sharedInstance].networkState=@"yes";
...
+(URLSessionTask *)postRequestUrl:(NSString *)url params:(NSDictionary *)params cache:(BOOL )cache target:(UIViewController *)target indicator:(BOOL)indicator progressBlock:(RequestProgress)progressBlock successBlock:(RequestSuccess)successBlock failBlock:(RequestFail)failBlock
{
    ...
    if (responseObj && cache) {
        if (successBlock) {
            // if ((isNotNull(responseObj[@"errCode"])) &&([responseObj[@"errCode"] isEqualToString:@"FORCE_LOG_OUT"])) {
            // NSNotification *notification=[NSNotification notificationWithName:@"loginout" object:nil];
            //[[NSNotificationCenter defaultCenter]postNotification:notification];
            //[SVProgressHUD dismiss];
            //return nil;
...
-(NSString *)urlDictToStringWithUrlStr:(NSString *)urlStr WithDict:(NSDictionary *)parameters
{
    ...
    [muDic removeObjectForKey:@"stime"];
    [muDic removeObjectForKey:@"sign"];
    [muDic removeObjectForKey:@"token"];
...
+ (NSMutableDictionary*)getBaseParameter{
    ...
    // [parameter setValue:timeNow forKey:@"token"];
    
    
    //       [parameter setObject:@"1920160628000001" forKey:@"userId"];
    //    [parameter setObject:@"3700020170420000013" forKey:@"userId"];
    //[parameter setObject:[LoginModel share].userId forKey:@"userId"];
